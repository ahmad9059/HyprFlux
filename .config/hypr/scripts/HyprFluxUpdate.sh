...
HyprFlux_Dots_DIR="$HOME/Hyprland-Dots"
  ...
  case "$response" in
    "action1")  
      if [ -d $HyprFlux_Dots_DIR ]; then
        ...
        kitty -e bash -c "
          git clone --depth=1 https://github.com/JaKooLit/Hyprland-Dots.git $HyprFlux_Dots_DIR &&
          cd $HyprFlux_Dots_DIR &&
          chmod +x copy.sh &&
          ./copy.sh &&
		  notify-send -u critical -i "$iDIR/ja.png" 'Update Completed:' 'Kindly log out and relogin to take effect'
