...
animate_slide_down() {
  ...
  hyprctl eval "hl.dispatch(hl.dsp.window.move({ x = $target_x, y = $start_y, window = 'address:$addr' }))" >/dev/null 2>&1
  sleep 0.05

  # Animate slide down
  for i in $(seq 1 $SLIDE_STEPS); do
    local current_y=$((start_y + (step_y * i)))
    hyprctl eval "hl.dispatch(hl.dsp.window.move({ x = $target_x, y = $current_y, window = 'address:$addr' }))" >/dev/null 2>&1
    sleep 0.03
  done

  # Ensure final position is exact
  hyprctl eval "hl.dispatch(hl.dsp.window.move({ x = $target_x, y = $target_y, window = 'address:$addr' }))" >/dev/null 2>&1
}
...
animate_slide_up() {
  ...
  for i in $(seq 1 $SLIDE_STEPS); do
    local current_y=$((start_y - (step_y * i)))
    hyprctl eval "hl.dispatch(hl.dsp.window.move({ x = $start_x, y = $current_y, window = 'address:$addr' }))" >/dev/null 2>&1
    sleep 0.03
  done
...
spawn_terminal() {
  ...
  hyprctl eval "hl.exec_cmd('$TERMINAL_CMD', { float = true, size = { $width, $height }, workspace = 'special:scratchpad silent' })"
  ...
  if [ -n "$new_addr" ] && [ "$new_addr" != "null" ]; then
    ...
    hyprctl eval "hl.dispatch(hl.dsp.window.move({ workspace = '$CURRENT_WS silent', window = 'address:$new_addr' }))"
    hyprctl eval "hl.dispatch(hl.dsp.window.pin({ window = 'address:$new_addr' }))"
...
if terminal_exists; then
  ...
  if terminal_in_special; then
    ...
    # Use movetoworkspacesilent to avoid affecting workspace history
    hyprctl eval "hl.dispatch(hl.dsp.window.move({ workspace = '$CURRENT_WS silent', window = 'address:$TERMINAL_ADDR' }))"
    hyprctl eval "hl.dispatch(hl.dsp.window.pin({ window = 'address:$TERMINAL_ADDR' }))"

    # Set size and animate slide down
    hyprctl eval "hl.dispatch(hl.dsp.window.resize({ x = $width, y = $height, window = 'address:$TERMINAL_ADDR' }))"
    animate_slide_down "$TERMINAL_ADDR" "$target_x" "$target_y" "$width" "$height"

    hyprctl eval "hl.dispatch(hl.dsp.focus({ window = 'address:$TERMINAL_ADDR' }))"
  else
    ...
    if [ -n "$geometry" ]; then
      ...
      sleep 0.1
      hyprctl eval "hl.dispatch(hl.dsp.window.pin({ window = 'address:$TERMINAL_ADDR' }))" # Unpin (toggle)
      hyprctl eval "hl.dispatch(hl.dsp.window.move({ workspace = '$SPECIAL_WS silent', window = 'address:$TERMINAL_ADDR' }))"
    else
