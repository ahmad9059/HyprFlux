...
copy_phase2() {
  local log="$1"
  local DIR="cava fastfetch hypr kitty Kvantum qt5ct qt6ct rofi swaync waybar wlogout"
...
cleanup_duplicate_userconfigs() {
  ...
  # HyprFlux: config is Lua-based; this cleanup only ever ran for legacy
  # <= v2.3.19 .conf installs and is a no-op for v2.4.0+ (version-gated above).
...
" >&2
    ...
    if version_gte "$CURRENT_VERSION" "$TARGET_VERSION"; then
      ...
      for FILE_NAME in "${FILES_TO_RESTORE[@]}"; do
        ...
        if [ -f "$BACKUP_FILE" ]; then
          if [ "$FILE_NAME" = "startup-apps.lua" ] || [ "$FILE_NAME" = "window-rules.lua" ]; then
            echo "${OK:-[OK]} - Restored ${YELLOW:-}$FILE_NAME${RESET:-} directly (no overlay in Lua config)" 2>&1 | tee -a "$log"
